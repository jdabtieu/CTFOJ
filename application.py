...
def login():
    ...
    if rows[0]["banned"]:
        return render_template("login.html", message="You are banned! Please message an admin to appeal the ban."), 403
...
def register():
    ...
    if not request.form.get("password") or len(request.form.get("password")) < 8:
        return render_template("register.html", message="Password must be at least 8 characters"), 400
...
def changepassword():
    ...
    if not request.form.get("password"):
        return render_template("changepassword.html", message="Password cannot be blank"), 400
    if not request.form.get("newPassword") or lenrequest.form.get("newPassword") < 8:
        return render_template("changepassword.html", message="New password must be at least 8 characters"), 400
...
def reset_password_user(token):
    ...
    if not request.form.get("password") or len(request.form.get("password")) < 8:
        return render_template("resetpassword.html", message="New password must be at least 8 characters"), 400
    if not request.form.get("confirmation") or request.form.get("password") != request.form.get("confirmation"):
...
def contest_problem(contest_id, problem_id):
    ...
    if datetime.now() > end:
        return render_template("contest/contest_problem.html", data=check[0], status="fail", message="This contest has ended.")

    flag = request.form.get("flag")
    if not flag:
        return render_template("contest/contest_problem.html", data=check[0], status="fail", message="Cannot submit an empty flag!")
...
def contest_add_problem(contest_id):
    ...
    if datetime.now() > end:
        return render_template("admin/createproblem.html", message="This contest has already ended!"), 403
    ...
    if not request.form.get("id") or not request.form.get("name") or not request.form.get("description") or not request.form.get("point_value") or not request.form.get("category") or not request.form.get("flag"):
        return render_template("admin/createproblem.html", message="You have not entered all required fields"), 400
...
def problem(problem_id):
    ...
    if not request.form.get("flag"):
        return render_template('problem/problem.html', data=data[0], status="fail", message="You did not enter a flag"), 400
...
def editcontest(contest_id):
    ...
    if not new_name:
        return render_template('admin/editcontest.html', data=data[0], message="Name cannot be empty"), 400
    if not new_description:
        return render_template('admin/editcontest.html', data=data[0], message="Description cannot be empty"), 400

...
def test_login(client, database):
    ...
    database.execute(
        ("INSERT INTO 'users' VALUES(2, 'user', 'pbkdf2:sha256:150000$XoLKRd3I$"
         ...
         "datetime('now'), 1, 1, 0, NULL, 0, 0, 0, 0)"))
    result = client.post('/login', data={
        'username': 'user',
        'password': 'CTFOJadmin'
    }, follow_redirects=True)
    ...
    database.execute(
        ("INSERT INTO 'users' VALUES(3, 'user2', 'pbkdf2:sha256:150000$XoLKRd3I$"
         "2dbdacb6a37de2168298e419c6c54e768d242aee475aadf1fa9e6c30aa02997f', 'e3', "
         "datetime('now'), 0, 0, 0, NULL, 0, 0, 0, 0)"))
    result = client.post('/login', data={
        'username': 'user2',
        'password': 'CTFOJadmin'
    }, follow_redirects=True)
    ...
    result = client.get('/admin/console', follow_redirects=True)
    assert result.status_code == 200
